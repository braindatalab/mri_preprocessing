...
FreeSurfer-related statistics can be generated as csv files with the shell script [`freesurfer_stats_generation.sh`](scripts/fs_stats_generation.sh). The files generated by the script can be combined into one single dataframe using the notebook [`merge_fs_stats_files.ipynb`](misc/merge_fs_stats_files.ipynb).
